...
import { PlayButton, PauseButton, ForwardButton, ReverseButton,
   ElapsedTimeValue, ElapsedTimeRange,
...
export default class Controller extends React.Component {
  onClick(buttonType){
    ...
    switch (buttonType) {
      case 'zoom-in': {
        updateViewState({...viewState, zoom:(viewState.zoom+0.5)})
        break
      }
      case 'zoom-out': {
        updateViewState({...viewState, zoom:(viewState.zoom-0.5)})
  ...
  render() {
    ...
    const { actions, inputFileName, animatePause, animateReverse, leading,
      settime, timeBegin, timeLength, secperhour, clusterNum, setClusterNum } = this.props;
